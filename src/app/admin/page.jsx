...
export default function AdminPage() {
  ...
  async function handleAccessModeChange(mode) {
    ...
    try {
      ...
      setAccessModeState(mode);
      setMessage(`Access mode changed to ${mode}`);
    } catch (error) {
  ...
  async function handleCleanup() {
    ...
    try {
      const deletedCount = await cleanupOldData(3);
      setMessage(`Cleanup completed. Deleted ${deletedCount} old records.`);
    } catch (error) {
  ...
  async function handleResetCounter() {
    const confirmReset = window.confirm(
      "Are you sure? Token number will start again from 1."
    );
  ...
  if (!isAdmin) {
    return (
      <AppShell
        title="Admin"
        subtitle="Only admin account can access this page."
      >
  ...
  return (
    <AppShell
      title="Admin"
      subtitle="Manage access, users, cleanup and token counter."
    >
      <div className="space-y-5">
        <div className="rounded-[30px] border border-[#eadfce] bg-[#fffaf3] p-5 shadow-[0_12px_35px_rgba(90,64,43,0.08)]">
          ...
          <h2 className="mt-2 text-2xl font-black text-[#2f241d]">
            Current: {accessMode === "open" ? "Open Link" : "Login Required"}
          </h2>
        ...
        <div className="rounded-[30px] border border-[#eadfce] bg-[#fffaf3] p-5 shadow-[0_12px_35px_rgba(90,64,43,0.08)]">
          ...
          <div className="mt-4 space-y-3">
            {users.map((user) => (
              <div
                key={user.loginId}
                className="rounded-2xl bg-[#f8f0e7] p-4"
              >
        ...
        <div className="rounded-[30px] border border-[#eadfce] bg-[#fffaf3] p-5 shadow-[0_12px_35px_rgba(90,64,43,0.08)]">
          ...
          <p className="mt-2 text-sm leading-6 text-[#715b48]">
            This deletes records and photos older than 3 days.
          </p>

          <div className="mt-4">
            <LoadingButton
              loading={loadingCleanup}
              loadingText="Cleaning old data..."
